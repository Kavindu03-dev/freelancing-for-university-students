  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      const updateData = {
        ...
        staffRole: editFormData.staffRole,
  ...
  const handleProfileImageRemove = async () => {
    try {
      ...
      if (response.ok) {
        const result = await response.json();
        if (result.success) {
          // Update staff data to remove profile image
          setStaffData(prev => ({
            ...prev,
            profileImage: null
          }));
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-4 py-3 border-2 rounded-xl focus:outline-none focus:ring-4 focus:ring-blue-200 ${
                        editErrors.staffRole ? 'border-red-500' : 'border-gray-300 focus:border-blue-500'
                      }`}
                      placeholder="e.g., Academic Advisor, Career Counselor"
                    />
                    {editErrors.staffRole && <p className="text-red-500 text-sm mt-1">{editErrors.staffRole}</p>}
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Department *</label>
                    <input
                      ...
                      className={`w-full px-4 py-3 border-2 rounded-xl focus:outline-none focus:ring-4 focus:ring-blue-200 ${
                        editErrors.department ? 'border-red-500' : 'border-gray-300 focus:border-blue-500'
                      }`}
                      placeholder="e.g., Computer Science, Student Services"
                    />
                    {editErrors.department && <p className="text-red-500 text-sm mt-1">{editErrors.department}</p>}
                  </div>
                  <div>
